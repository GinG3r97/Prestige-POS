-- Settling a ₱0 tab (free/comped items) violated payments_amount_cents_check
-- (amount_cents > 0). Only record a payment when there is money to record;
-- the order is still marked paid either way.
-- Signature unchanged (uuid[], text) → in-place replace, no overload created.
