...
// ───── Categories tab (built-in read-only + custom CRUD) ─────
...
class _CategoriesTab extends StatelessWidget {
  ...
  Future<void> _openForm(
      BuildContext context, AppState state, cat.Category? existing) async {
    final saved = await showDialog<cat.Category>(
      context: context,
      builder: (_) => _CategoryDialog(initial: existing),
    );
    if (saved == null || !context.mounted) return;
    final err = existing == null
  ...
  Future<void> _confirmRemove(
      BuildContext context, AppState state, cat.Category category) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
    ...
    if (ok != true || !context.mounted) return;
    final err = await state.removeCategory(category.id);
    if (!context.mounted) return;
    if (err != null) {
      PushToast.show(context,
          title: 'Could not remove',
...
class _ProductTypesTab extends StatelessWidget {
  const _ProductTypesTab({required this.state});
  final AppState state;
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 2),
                      Text(
                        'Behavior buckets each product belongs to. Toggles control whether the type supports modifiers and deducts from inventory.',
                        style: YFont.caption(),
                      ),
                    ],
                  ),
                ...
                const SizedBox(width: 24),
                ElevatedButton.icon(
                  onPressed: () => _openTypeForm(context, state, null),
                  icon: const Icon(Icons.add, size: 18),
              ...
              if (types.isEmpty)
                _emptyCard(
                  ...
                  title: 'No product types yet',
                  subtitle:
                      'Add a type like Drink, Food, or Book to start adding products.',
                )
              else
                LayoutBuilder(builder: (ctx, c) {
                  final cols = c.maxWidth > 900
                  ...
                  return GridView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return _MaintGridTile(
                        ...
                        label: t.name,
                        subtitle: t.behaviorSummary,
                        badge: t.isSystem ? 'BUILT-IN' : null,
                        onTap: () => _openTypeForm(context, state, t),
                        onRemove: () =>
                            _confirmRemoveType(context, state, t),
                      );
                    },
                  );
                }),
  ...
  Future<void> _openTypeForm(
      BuildContext context, AppState state, ProductType? existing) async {
    final saved = await showDialog<ProductType>(
      context: context,
      barrierDismissible: false,
      builder: (_) => _ProductTypeDialog(initial: existing),
    );
    if (saved == null || !context.mounted) return;
    final err = existing == null
        ? await state.addProductType(saved)
    ...
    PushToast.show(context,
        title: existing == null ? 'Type added' : 'Type updated',
        subtitle: saved.name,
        leadingIcon: Icons.label_outline);
  }

  Future<void> _confirmRemoveType(
      BuildContext context, AppState state, ProductType t) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: const Text(
            'Products using this type will become untyped — the owner has to reassign them.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text('Cancel')),
